...
function FAQ() {
  ...
  return (
    <div className="faq-container" style={{marginLeft:"0px"}}>
          <span style={{ 
      width: '676px',
      height: '39px',
...
<br></br>
      {faqs.map((faq) => (
        <div key={faq.id} className="faq-item">
          <div
            className={`faq-question ${faq.id === activeQuestion ? 'active' : ''}`}
            onClick={() => toggleQuestion(faq.id)}>
            {faq.question}
            <i className={"fa fa-angle-down"} style={{ position:"absolute",left: '620px' }}></i>
          </div>
